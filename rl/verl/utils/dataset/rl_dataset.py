...
class RLHFDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_multiturn = config.get("use_multiturn", False)
                ...
                def doc2len(doc) -> int:
                    try:
                        ...
                        if self.use_chat_template:
                            return len(tokenizer.apply_chat_template(prompt, add_generation_prompt=True, **apply_kwargs))
                        # no-template: prompt can be str or list[dict]
    ...
    def __getitem__(self, item):
        ...
        if self.processor is not None:
            ...
            if self.use_chat_template:
                ...
                # no template: messages is list[dict] OR prompt could be already a string
        ...
        env_replies = row_dict.get("extra_info", {}).get("env_replies", '')
        if need_tools_kwargs and not tools_kwargs:
            logger.warning("tools_kwargs is empty for index {}, data source: {}", index, row_dict["data_source"])
        row_dict["index"] = index
        row_dict["tools_kwargs"] = tools_kwargs
        row_dict["interaction_kwargs"] = interaction_kwargs
        row_dict["env_replies"] = env_replies
        import ast
        if isinstance(env_replies, str) and env_replies.strip():
            env_replies = ast.literal_eval(env_replies)
        else:
            env_replies = []
        return row_dict
