...
def _get_event_loop():
    """Get the current event loop, creating one if none exists.

    Under uvloop (installed with sglang) on Python 3.10+, asyncio.get_event_loop()
    raises "There is no current event loop in thread 'MainThread'" instead of
    auto-creating one. These sync rollout entrypoints run in a Ray worker thread
    with no running loop, so create and install one on first use.
    """
    try:
...
class SGLangRollout(BaseRollout):
    ...
    def _init_inference_engine(self, trust_remote_code, actor_module, port):
        ...
        if first_rank_in_node:
            ...
            self._engine = AsyncEngine(
                ...
                # Disable the grammar/constrained-decoding backend. SGLang eagerly
                # builds XGrammar at scheduler init, which calls
                # TokenizerInfo.from_huggingface and raises
                # "Unsupported tokenizer type: HFTokenizerWrapper" on the custom
                # chess tokenizer, killing the scheduler subprocess. We never use
                # structured output (no json_schema/regex/ebnf), so skip it.
    ...
    def _batch_level_generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Bridge vLLM-style per-call kwargs emitted by the interactive/multi-turn
        # worker loop (fsdp_workers.generate_multi_turn_sequences) to SGLang:
        #   - vLLM uses `max_tokens`; SGLang's SamplingParams uses `max_new_tokens`.
        #     Without this, the per-round token budget is dropped and each round
        #     falls back to the default max_new_tokens.
        #   - SGLang trims the matched stop token from the output by default
        #     (no_stop_trim=False). The multi-turn loop detects a turn boundary via
        #     `call_env_id in response_tokens` and cuts INCLUDING <call_env>, so the
        #     stop token must be retained or multi-turn collapses to single-turn.
        if "max_tokens" in request_sampling_params:
            request_sampling_params["max_new_tokens"] = request_sampling_params.pop("max_tokens")
        if request_sampling_params.get("stop_token_ids"):
            request_sampling_params["no_stop_trim"] = True

        # Clamp the completion length so (input + max_new_tokens) never exceeds the
        # model's context window. vLLM clamps max_tokens silently; SGLang instead
        # raises ValueError("Requested token count exceeds the model's maximum
        # context length"). The multi-turn loop sizes each round's budget against
        # response_length and relies on the engine to clamp, so do it here. Engine
        # generation is batched with one sampling param, so clamp to the longest
        # input in the batch (post-generation trimming is per-sample already).
        if "max_new_tokens" in request_sampling_params and idx_list:
            longest_input = max(len(ids) for ids in idx_list)
            room = self.config.max_model_len - longest_input - 1
            if room > 0:
                request_sampling_params["max_new_tokens"] = min(
                    int(request_sampling_params["max_new_tokens"]), room
                )
